...
export class ExplorerApi {
  ...
  public static async create(nodeUrl: string, neutrinoContractAddress: string) {
    const accountDataState = await nodeInteraction.accountData(
      neutrinoContractAddress,
      nodeUrl
    );

    const auctionContractAddress = <string>(
      accountDataState[NeutrinoContractKeys.AuctionContractAddressKey].value
    );
    const controlContractAddress = <string>(
      accountDataState[NeutrinoContractKeys.ControlContractAddressKey].value
    );
    const liquidationContractAddress = <string>(
      accountDataState[NeutrinoContractKeys.LiquidationContractAddressKey].value
    );
    const rpdContractAddress = <string>(
      accountDataState[NeutrinoContractKeys.RpdContractAddressKey].value
    );

    const neutrinoAssetId = <string>(
      accountDataState[NeutrinoContractKeys.NeutrinoAssetIdKey].value
    );
    const bondAssetId = <string>(
      accountDataState[NeutrinoContractKeys.BondAssetIdKey].value
    );
  ...
  private async getAssetQuantity() {
    const assetObject = await axios.get(
      this.nodeUrl + "assets/details/" + this.neutrinoAssetId
    );
  ...
  private async getNeutrinoLockedBalance(): Promise<number> {
    return <number>(
      (
        await nodeInteraction.accountDataByKey(
          AuctionContractKeys.WavesLockedBalanceKey,
          this.neutrinoContractAddress,
          this.nodeUrl
        )
      ).value
    );
  ...
  private getSmartContractAddresses() {
    console.log(
      ...
      this.controlContractAddress,
      this.liquidationContractAddress,
      this.rpdContractAddress
    );
  }
  ...
  public async getPriceBlocks(start, end): Promise<any> {
    ...
    for (let i = startBlock; i <= endBlock; i++) {
      let key = prefix + i;
      let priceObject = await nodeInteraction.accountDataByKey(
        key,
        this.controlContractAddress,
        this.nodeUrl
      );
  ...
  public async getTotalIssued(): Promise<number> {
    ...
    const neutrinoBalance =
      (await this.getNeutrinoBalance(this.neutrinoContractAddress)) /
      10 ** assetDecimals;
    const liquidationBalance =
      (await this.getNeutrinoBalance(this.liquidationContractAddress)) /
      10 ** assetDecimals;
    const balanceLockNeutrino =
      Number(
        (
          await nodeInteraction.accountDataByKey(
            "balance_lock_neutrino",
            this.neutrinoContractAddress,
            this.nodeUrl
          )
        ).value
  ...
  public async getStaked(): Promise<number> {
    return (
      <number>(
        (
          await nodeInteraction.accountDataByKey(
            RpdContractKeys.BalanceKey + "_" + this.neutrinoAssetId,
            this.rpdContractAddress,
            this.nodeUrl
          )
        ).value
      ) /
      10 ** (await this.getDecimals())
    );
  }
  ...
  public async getAnnualYield(): Promise<number> {
    ...
    const txObject = await axios.get(
      this.nodeUrl + "transactions/address/" + stakingAddress + "/limit/99"
    );
    const txData = txObject.data[0];

    const filteredTxData = txData
      .filter(
        (item) =>
          String(item.sender) === String("3PLosK1gb6GpN5vV7ZyiCdwRWizpy2H31KR")
      )
      .slice(0, averageDays);
  ...
  public async getCirculatingSupplyNoDec(): Promise<number> {
    return <number>(
      (((await this.getTotalIssued()) - (await this.getStaked())) *
        10 ** (await this.getDecimals()))
    );
  }
  ...
  public async getLockedForSwap(): Promise<number> {
    let lockedForSwapPauli = await axios.get(
      this.nodeUrl +
        "addresses/data/" +
        this.neutrinoContractAddress +
        "/" +
  ...
  public async getDeficitPerCent(): Promise<number> {
    return (
      <number>(
        (-1 * ((await this.getDeficit()) / (await this.getTotalIssued())))
      ) * 100
    );
  }

  public async getTotalBondsRest(): Promise<number> {
    let orders_object = await axios.get(
      "https://beta.neutrino.at/api/v1/bonds/usd-nb_usd-n/orders"
    );
  ...
  public async getTotalLiquidation(): Promise<number> {
    let orders_object = await axios.get(
      "https://beta.neutrino.at/api/v1/liquidate/usd-nb_usd-n/orders"
    );
    let orders_object_data = orders_object.data;
